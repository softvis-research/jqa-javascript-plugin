...
import org.jqassistant.contrib.plugin.ecmascript.scanner.ECMAScriptSourceParser;
...
public class ECMAScriptFileScannerPlugin extends AbstractScannerPlugin<FileResource, ECMAScriptFileDescriptor> {
    
    private static final Logger LOGGER = LoggerFactory.getLogger(ECMAScriptFileScannerPlugin.class);
     ...
     */
    @Override
    public ECMAScriptFileDescriptor scan(final FileResource item, final String path, final Scope scope, final Scanner scanner) throws IOException {
        final Store store = scanner.getContext().getStore();
        final ECMAScriptSourceParser sourceParser = new ECMAScriptSourceParser (store);
